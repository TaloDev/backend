  ...
  if (isAPIRoute(ctx) && (ctx.state.currentPlayerId || ctx.state.currentAliasId)) {
    ...
    if (ctx.state.currentPlayerId) {
      alias = await em.getRepository(PlayerAlias).findOne({
        ...
        ...(getResultCacheOptions(getAuthMiddlewarePlayerKey(ctx.state.currentPlayerId), 10_000) ?? {}),
      ...
      alias = await em.getRepository(PlayerAlias).findOne({
        ...
        ...(getResultCacheOptions(getAuthMiddlewareAliasKey(ctx.state.currentAliasId), 10_000) ?? {}),
        populate: ['player.auth']
      })
    }
