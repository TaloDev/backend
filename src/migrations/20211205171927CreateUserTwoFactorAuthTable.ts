...
export class CreateUserTwoFactorAuthTable extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table `user_two_factor_auth` (`id` int unsigned not null auto_increment primary key, `secret` varchar(255) not null, `enabled` tinyint(1) not null) default character set utf8mb4 engine = InnoDB;',
    )
    ...
    this.addSql(
      'alter table `user` add unique `user_two_factor_auth_id_unique`(`two_factor_auth_id`);',
    )
