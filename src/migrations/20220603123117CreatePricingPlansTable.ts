...
export class CreatePricingPlansTable extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table `pricing_plan` (`id` int unsigned not null auto_increment primary key, `stripe_id` varchar(255) not null, `hidden` tinyint(1) not null default false, `default` tinyint(1) not null default false, `created_at` datetime not null, `updated_at` datetime not null) default character set utf8mb4 engine = InnoDB;',
    )
    ...
    this.addSql(
      'alter table `organisation` add unique `organisation_pricing_plan_id_unique`(`pricing_plan_id`);',
    )
  }

  override async down(): Promise<void> {
    this.addSql(
      'alter table `pricing_plan_action` drop foreign key `pricing_plan_action_pricing_plan_id_foreign`;',
    )
    ...
    this.addSql(
      'alter table `organisation_pricing_plan_action` drop foreign key `organisation_pricing_plan_action_organisation_pri_a08fe_foreign`;',
    )
