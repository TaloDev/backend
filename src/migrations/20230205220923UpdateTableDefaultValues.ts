...
export class UpdateTableDefaultValues extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      "alter table `organisation_pricing_plan` modify `status` varchar(255) not null default 'active';",
    )

    this.addSql(
      "alter table `player_group` modify `rule_mode` enum('$and', '$or') not null default '$and';",
    )
    ...
    this.addSql(
      "alter table `leaderboard` modify `sort_mode` enum('desc', 'asc') not null default 'desc';",
    )
