...
export class CreateGameFeedbackAndCategoryTables extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table `game_feedback_category` (`id` int unsigned not null auto_increment primary key, `internal_name` varchar(255) not null, `name` varchar(255) not null, `description` varchar(255) not null, `anonymised` tinyint(1) not null, `game_id` int unsigned not null, `created_at` datetime not null, `updated_at` datetime not null) default character set utf8mb4 engine = InnoDB;',
    )
    this.addSql(
      'alter table `game_feedback_category` add index `game_feedback_category_game_id_index`(`game_id`);',
    )
    ...
    this.addSql(
      'alter table `game_feedback` add index `game_feedback_category_id_index`(`category_id`);',
    )
    this.addSql(
      'alter table `game_feedback` add index `game_feedback_player_alias_id_index`(`player_alias_id`);',
    )
