...
export class CreateGameChannelPropTable extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      'create table `game_channel_prop` (`id` int unsigned not null auto_increment primary key, `game_channel_id` int unsigned not null, `key` varchar(128) not null, `value` varchar(512) not null, `created_at` datetime not null) default character set utf8mb4 engine = InnoDB;',
    )
    this.addSql(
      'alter table `game_channel_prop` add index `game_channel_prop_game_channel_id_index`(`game_channel_id`);',
    )
