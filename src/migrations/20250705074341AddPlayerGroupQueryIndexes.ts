...
export class AddPlayerGroupQueryIndexes extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql('alter table `leaderboard_entry` add index `leaderboard_entry_hidden_index`(`hidden`);')
    this.addSql('alter table `leaderboard_entry` add index `idx_leaderboardentry_hidden_leaderboard_id_score`(`hidden`, `leaderboard_id`, `score`);')

    this.addSql('alter table `game_stat` add index `game_stat_internal_name_index`(`internal_name`);')

    this.addSql('alter table `player_game_stat` add index `idx_playergamestat_stat_id_value`(`stat_id`, `value`);')
  ...
  override async down(): Promise<void> {
    ...
    this.addSql('alter table `leaderboard_entry` drop index `idx_leaderboardentry_hidden_leaderboard_id_score`;')
