...
export class CreateSteamworksLeaderboardEntryTable extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      'create table `steamworks_leaderboard_entry` (`id` int unsigned not null auto_increment primary key, `steamworks_leaderboard_id` int unsigned not null, `leaderboard_id` int unsigned not null, `leaderboard_entry_id` int unsigned null, `steam_user_id` varchar(255) not null, `created_at` datetime not null, `updated_at` datetime not null) default character set utf8mb4 engine = InnoDB;',
    )
    this.addSql(
    ...
    this.addSql(
      'alter table `steamworks_leaderboard_entry` add index `steamworks_leaderboard_entry_steamworks_leaderboard_aa6b4_index`(`steamworks_leaderboard_id`, `leaderboard_id`);',
    )
