import PlayerAuthResetPassword from '../../src/emails/player-auth-reset-password-mail.js'
import PlayerAuthFactory from '../fixtures/PlayerAuthFactory.js'
import PlayerFactory from '../fixtures/PlayerFactory.js'
import createOrganisationAndGame from '../utils/createOrganisationAndGame.js'

describe('PlayerAuthResetPassword mail', () => {
  ...
  it('shows the game logo and links to the game website when both are set', async () => {
    ...
    const mail = new PlayerAuthResetPassword(alias, '123456')
  ...
  it('sets the recipient, subject, title and main text from the alias and game', async () => {
    ...
    const code = '123456'
    const mail = new PlayerAuthResetPassword(alias, code)

    expect(mail.to).toBe('boz@mail.com')
    expect(mail.subject).toBe(`Reset your ${alias.player.game.name} password`)
    expect(mail.title).toBe('Reset your password')
    expect(mail.mainText).toContain(alias.identifier)
    expect(mail.mainText).toContain(code)
    expect(mail.mainText).toContain(alias.player.game.name)
