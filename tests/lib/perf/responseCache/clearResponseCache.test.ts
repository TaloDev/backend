...
describe('clearResponseCache', () => {
  ...
  it('should catch redis errors', async () => {
    const evalSpy = vi.spyOn(Redis.prototype, 'eval').mockRejectedValueOnce(new Error())
    await redis.set(`${prefix}:key1`, 'key1')
    await redis.set(`${prefix}:key2`, 'key2')

    const res = await clearResponseCache(redis, 'key*')
    expect(res).toBe(0)
