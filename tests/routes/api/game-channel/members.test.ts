...
describe('Game channel API - members', () => {
  ...
  it('should paginate channel members', async () => {
    ...
    const channel = await new GameChannelFactory(apiKey.game).one()
    const player = await new PlayerFactory([apiKey.game]).one()
    channel.members.set([player.aliases[0], ...(await new PlayerAliasFactory(player).many(100))])
    await em.persist(channel).flush()
  ...
  it('should filter by player prop keys', async () => {
    ...
    const player = await new PlayerFactory([apiKey.game])
      .state((player) => ({
        props: new Collection<PlayerProp>(player, [new PlayerProp(player, 'collectibles', '8')]),
      }))
      .one()
    const otherPlayer = await new PlayerFactory([apiKey.game]).one()
    const channel = await new GameChannelFactory(apiKey.game).one()
  ...
  it('should filter by player groups', async () => {
    ...
    const player = await new PlayerFactory([apiKey.game])
      .state((player) => ({
        props: new Collection<PlayerProp>(player, [new PlayerProp(player, 'someRandomProp', '60')]),
      }))
      .one()
    const otherPlayer = await new PlayerFactory([apiKey.game]).one()
    const channel = await new GameChannelFactory(apiKey.game).one()
