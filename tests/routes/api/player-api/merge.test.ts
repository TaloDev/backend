...
describe('Player API  - merge', () => {
  ...
  it('should correctly replace properties in player1 with player2\'s', async () => {
    ...
    const player1 = await new PlayerFactory([apiKey.game]).withSteamAlias().state((player) => ({
    ...
    const player2 = await new PlayerFactory([apiKey.game]).withUsernameAlias().state((player) => ({
      props: new Collection<PlayerProp>(player, [
        new PlayerProp(player, 'currentLevel', '60'),
  ...
  it('should not merge if both players have aliases with the same service', async () => {
    ...
    const steamAlias1 = await new PlayerAliasFactory(player1).steam().state(() => ({
      identifier: 'player1_steam_id',
      lastSeenAt: new Date('2024-01-01')
    })).one()

    const steamAlias2 = await new PlayerAliasFactory(player2).steam().state(() => ({
    ...
    expect(res.body).toStrictEqual({
      message: 'Cannot merge players: both players have aliases with the following service(s): steam'
    })
  })
})
