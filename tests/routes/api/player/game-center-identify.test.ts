...
import { PlayerAliasService } from '../../../../src/entities/player-alias.js'
import { clearCertCache } from '../../../../src/lib/integrations/clients/game-center-client.js'
...
const publicKeyUrl = 'https://static.gc.apple.com/public-key/gc-prod-12.cer'
...
function makeIdentifier(
  ...
  const playerId = overrides.playerId ?? 'T:_fffbbbddd'
  const resolvedBundleId = overrides.bundleId ?? bundleId
  const timestamp = overrides.timestamp ?? Date.now()
  const salt = Buffer.from('random-salt')

  const signature = overrides.signature ?? signPayload(playerId, resolvedBundleId, timestamp, salt)
...
describe('Player API - Game Center identify', () => {
  ...
  it('should identify a non-existent game center player by creating a new player with the write scope', async () => {
    const playerId = 'T:_fffaaaccchhh'
