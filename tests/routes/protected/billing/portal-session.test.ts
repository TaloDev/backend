...
describe('Billing - create portal session', () => {
    ...
    async (statusCode, _, type) => {
      ...
      if (statusCode === 200) {
        ...
        expect(res.body).toStrictEqual({
          message: 'You do not have permissions to update the organisation pricing plan',
        })
      }
    },
  )
  ...
  it("should return a 400 if the organisation doesn't have a stripeCustomerId", async () => {
    ...
    await request(app).post('/billing/portal-session').auth(token, { type: 'bearer' }).expect(400)
  })
  ...
  it('should return a 405 with no stripe key', async () => {
    ...
    await request(app).post('/billing/portal-session').auth(token, { type: 'bearer' }).expect(405)
