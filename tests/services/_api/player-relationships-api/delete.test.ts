...
describe('Player subscriptions API service - delete', () => {
  ...
  it('should not delete a subscription if the alias does not exist', async () => {
    ...
    const res = await request(app)
      .delete('/v1/players/relationships/some-id')
      .auth(token, { type: 'bearer' })
  ...
  it('should not delete a subscription that does not exist', async () => {
    ...
    const res = await request(app)
      .delete('/v1/players/relationships/non-existent-id')
      .auth(token, { type: 'bearer' })
  ...
  it('should delete unidirectional subscription without looking for reciprocal', async () => {
    const [apiKey, token] = await createAPIKeyAndToken([APIKeyScope.WRITE_PLAYER_RELATIONSHIPS])
    const player1 = await new PlayerFactory([apiKey.game]).one()
    const player2 = await new PlayerFactory([apiKey.game]).one()
    await em.persistAndFlush([player1, player2])
  ...
  it('should delete both subscriptions when deleting bidirectional relationship', async () => {
    const [apiKey, token] = await createAPIKeyAndToken([APIKeyScope.WRITE_PLAYER_RELATIONSHIPS])
    const player1 = await new PlayerFactory([apiKey.game]).one()
    const player2 = await new PlayerFactory([apiKey.game]).one()
    await em.persistAndFlush([player1, player2])
