...
describe('Player subscriptions API service - post', () => {
  ...
  it('should not create a subscription if the aliasId is not a number', async () => {
    ...
    const res = await request(app)
      .post('/v1/players/relationships')
      .send({ aliasId: 'not-a-number', relationshipType: 'unidirectional' })
      .auth(token, { type: 'bearer' })
      .set('x-talo-alias', String(player.aliases[0].id))
      .expect(400)

    expect(res.body.errors.aliasId).toStrictEqual(['aliasId must be a number'])
  })
  ...
  it('should not create a subscription if relationshipType is invalid', async () => {
    const [apiKey, token] = await createAPIKeyAndToken([APIKeyScope.WRITE_PLAYER_RELATIONSHIPS])
    const player1 = await new PlayerFactory([apiKey.game]).one()
    const player2 = await new PlayerFactory([apiKey.game]).one()
    await em.persistAndFlush([player1, player2])
