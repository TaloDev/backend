...
describe('Player subscriptions API service - getSubscribers', () => {
  it('should get confirmed subscribers', async () => {
    ...
    const res = await request(app)
      .get('/v1/players/subscriptions/subscribers')
    ...
    const subscriberIds = res.body.subscribers.map((s: { id: number }) => s.id)
  ...
  it('should only return pending subscribers when confirmed filter is false', async () => {
    const [apiKey, token] = await createAPIKeyAndToken([APIKeyScope.READ_PLAYER_SUBSCRIPTIONS])
    const player1 = await new PlayerFactory([apiKey.game]).one()
    const player2 = await new PlayerFactory([apiKey.game]).one()
    const player3 = await new PlayerFactory([apiKey.game]).one()
    ...
    const res = await request(app)
      .get('/v1/players/subscriptions/subscribers?confirmed=false')
      .auth(token, { type: 'bearer' })
      .set('x-talo-alias', String(player1.aliases[0].id))
      .expect(200)
    ...
    expect(res.body.isLastPage).toBe(true)
    expect(res.body.subscribers[0].id).toBe(player2.aliases[0].id)
  })

  it('should only return confirmed subscribers when confirmed filter is true', async () => {
    const [apiKey, token] = await createAPIKeyAndToken([APIKeyScope.READ_PLAYER_SUBSCRIPTIONS])
    const player1 = await new PlayerFactory([apiKey.game]).one()
    const player2 = await new PlayerFactory([apiKey.game]).one()
    const player3 = await new PlayerFactory([apiKey.game]).one()
    ...
    const res = await request(app)
      .get('/v1/players/subscriptions/subscribers?confirmed=true')
      .auth(token, { type: 'bearer' })
      .set('x-talo-alias', String(player1.aliases[0].id))
      .expect(200)
    ...
    expect(res.body.count).toBe(1)
    expect(res.body.isLastPage).toBe(true)
    expect(res.body.subscribers[0].id).toBe(player3.aliases[0].id)
  })
  ...
  it('should filter subscribers by aliasId', async () => {
    ...
    const res = await request(app)
      .get(`/v1/players/subscriptions/subscribers?aliasId=${player2.aliases[0].id}`)
      .auth(token, { type: 'bearer' })
      .set('x-talo-alias', String(player1.aliases[0].id))
      .expect(200)
  ...
  it('should return 400 if confirmed is invalid', async () => {
    ...
    const res = await request(app)
      .get('/v1/players/subscriptions/subscribers?confirmed=INVALID')
      .auth(token, { type: 'bearer' })
      .set('x-talo-alias', String(player.aliases[0].id))
      .expect(400)
  ...
  it('should return 400 if the aliasId is not a number', async () => {
    ...
    const res = await request(app)
      .get('/v1/players/subscriptions/subscribers?aliasId=notanumber')
      .auth(token, { type: 'bearer' })
      .set('x-talo-alias', String(player.aliases[0].id))
      .expect(400)
